...
extension State {
    func drive(events: Events) -> OutputStream {
        ...
        let nextStateOutputs: AnyAsyncSequence<AnyAsyncSequence<Output>> = nextState.map { $0.drive(events: events).eraseToAnyAsyncSequence() }.eraseToAnyAsyncSequence()

        let x: AnyAsyncSequence<AnyAsyncSequence<Output>> = nextStateOutputs.startWith(currentStateOutputs).eraseToAnyAsyncSequence()
